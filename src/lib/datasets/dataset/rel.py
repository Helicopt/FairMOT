...
class RelJointDataset(LoadImagesAndLabels):  # for training
    ...
    def __getitem__(self, files_index):
        ...
        def get_pattern(label_path):
            num = re.findall(r'[\d]+.txt$', label_path)
            if len(num) == 1:
                num = num[0]
                pattern = label_path[:-len(num)]
            elif 'Cityscapes' in label_path:
                pattern = re.sub(r'[\d]{6}', 'x6dx', label_path)
            else:
                assert 0, 'cannot find pattern for %s' % label_path
            return pattern

        img_path = self.img_files[ds][files_index - start_index]
        label_path = self.label_files[ds][files_index - start_index]
        pattern = get_pattern(label_path)
        options = []
        for i in range(files_index - 25, files_index + 25):
            if i - start_index < 0 or i - start_index >= len(self.label_files[ds]):
                continue
            tmp = self.label_files[ds][i - start_index]
            p = get_pattern(tmp)
            if p == pattern:
                options.append(i)
        assert len(options) > 0
        while len(options) < self.ref_num:
            options.append(options[-1])
        options = np.random.choice(options, size=self.ref_num)


        main = self._getitem(files_index)
        refs = []
        for i in range(self.ref_num):
            ind = int(options[i])
            refs.append(self._getitem(ind))

        refs = self._wrap(refs)
        ret = {}
        for k in main:
            if k in self.concerns:
                ret[k] = (main[k], refs[k])
            else:
                ret[k] = main[k]
        return ret
